...
puts "Il y a #{journalists.size} journalistes"
puts


puts "Combien d'handle contiennent un numéro ?"
puts "Il y a #{journalists.count{|x| x[/\d+/]}} handle."
puts


puts "Combien d'handle contiennent les 4 lettres du prénom 'Aude' à la suite (sans prendre en compte les majuscules) ?"
tableau_aude = []
tableau_aude += journalists.grep(/aude/)
tableau_aude += journalists.grep(/Aude/)
puts "#{tableau_aude}"
puts "Il y a #{tableau_aude.size} handles qui contiennent aude."
puts


puts "Combien commencent par une majuscule (première lettre juste après le @) ?"
puts "Il y a #{journalists.count {|x| x[/@+\p{Upper}/] }} handles qii commencent par une majuscule."
puts
...
tableau_ = []
tableau_ += journalists.grep(/_/)
puts "Il y a #{tableau_.size} handles qui contiennent '_'"
puts


puts "Trie la liste de handle par ordre alphabétique."
puts
print journalists.sort {|a,b| a <=> b}
...
puts "Les 50 handles les plus courts sont les suivants :"
puts journalists.sort_by {|word| word.length}
puts


puts "Quelle est la position dans l'array de la personne @epenser ?"
puts "La personne '@epenser' est à la position #{journalists.find_index("@epenser")}."
puts